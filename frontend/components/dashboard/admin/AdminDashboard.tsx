...
export function AdminDashboard() {
  ...
  async function toggle(user: User) {
    try {
      ...
      setUsers((current) =>
        current.map((x) => (x.id === user.id ? { ...x, status: next } : x)),
      );
    ...
    <div className="container-page py-10">
      <div className="mb-8">
        ...
        <p className="mt-2 text-slate-500">
          Monitor users and marketplace activity.
        </p>
      </div>
        ...
        {(
          [
            ...
            [
              Ban,
              "Banned users",
              users.filter((x) => x.status === "BANNED").length,
            ],
      ...
      <div className="mt-8 flex gap-2 border-b border-slate-200">
        {(["users", "properties", "rentals"] as Tab[]).map((tab) => (
          ...
          >
            ...
            {tab === "rentals" && (
              <span className="flex items-center gap-2">
                <FileText className="h-4 w-4" /> Rentals ({rentals.length})
              </span>
            )}
          </button>
        ))}
