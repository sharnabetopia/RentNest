...
export function LoginForm() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      window.location.href = role === "ADMIN" ? "/dashboard/admin" : role === "LANDLORD" ? "/dashboard/landlord" : "/dashboard/tenant";
    } catch (error) {
  ...
  return (
    <div className="container-page flex min-h-[calc(100vh-72px)] items-center justify-center py-10">
      <form onSubmit={submit} className="card w-full max-w-md p-7">
        ...
        <p className="mt-1 text-sm text-slate-500">Sign in to your RentNest account.</p>
        ...
        <p className="mt-5 text-center text-sm text-slate-500">Don&apos;t have an account? <Link href="/auth/register" className="font-semibold text-brand-700">Register</Link></p>
      </form>
    </div>
  );
}
