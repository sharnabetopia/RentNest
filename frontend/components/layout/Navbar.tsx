    ...
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
      <div className="container-page flex h-[72px] items-center justify-between">
        <Link href="/" className="flex items-center gap-2 font-bold text-slate-900">
          <span className="rounded-xl bg-brand-600 p-2 text-white"><Home className="h-5 w-5" /></span>
          <span className="text-xl">RentNest</span>
        </Link>

        <nav className="hidden items-center gap-6 md:flex">
          <Link href="/properties" className="text-sm font-medium text-slate-600 hover:text-brand-700">Properties</Link>
          {user ? (
            <>
            ...
            <>
              <Link href="/auth/login" className="flex items-center gap-1.5 text-sm font-medium text-slate-600"><LogIn className="h-4 w-4" /> Login</Link>
              <Link href="/auth/register" className="btn-primary"><UserPlus className="mr-2 h-4 w-4" /> Register</Link>
            </>
          )}
        </nav>
          ...
          <div className="container-page flex flex-col gap-4">
            ...
            {user ? (
              <>
                ...
                <button className="text-left text-red-600" onClick={() => { clearAuth(); window.location.href = "/"; }}>Logout</button>
              </>
            ) : (
              <>
